...
class WavefrontAlertToJiraMapperAction(Action):
    def run(self, 
            alert_name:str, 
            alert_severity:str, 
            alert_url:str, 
        ...
        self.source["alert_url"] = alert_url
        try:
    ...
    def __get_severity(self):
      ...
      if self.source["alert_severity"] == "SEVERE":
    ...
    def __get_affected_version(self):
      if "ws1version" in self.source:
        return jira_client.JiraClient.derive_affected_version(self.source["ws1version"])
      return "Future"